...
N개의 용접 생산라인은 제품 용접이 끝날 때마다 설비 PC 또는 NAS에 레이저 A, 레이저 B 데이터 CSV 파일을 저장한다.
Producer는 파일 묶음을 제품 단위로 감지하고 Kafka에 청크 단위로 발행한다.
...
| Producer | CSV 파일을 제품/lead/channel 단위로 그룹핑하고 raw topic에 발행 |
...
- N개 생산라인에서 동시에 데이터가 들어와도 producer를 독립적으로 운영할 수 있다.
